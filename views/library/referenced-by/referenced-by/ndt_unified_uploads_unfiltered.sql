--
-- This view, ndt_unified_uploads_unfilerd, exposes of all MLab upload
-- data across the entire platform over all time.  The schema uses a
-- superset of the Standard Top-level Columns design.  This view does
-- not filter tests, what-so-ever, but exposes predicates used for
-- filtering other viwes in struct (filter).
--
...
-- layering your queries such that data preening is done in
-- sub-queries or views, distinct from the research queries, such that
-- the data preening can be updated to follow changes to the
-- constituent views without refactoring the research.
--
